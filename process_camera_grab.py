...
async def grab_rtsp(vcap, loop, logger, cam, last_frame_id):
    ...
    try:
        ...
        logger.info(f"resultat de la lecture rtsp : {ret}  pour {cam['name']} with len "
                    f"{frame_id}")
        logger.info(f"last_frame_id is {last_frame_id} frame_id is {frame_id} --> frame diff is  {is_frame_diff}")

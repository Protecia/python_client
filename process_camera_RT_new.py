...
path = '/NNvision/tkDNN/build/room_detector_fp16.rt'
weights = path.encode()
net = {'all_RT': dn.load_network(weights, 15, 1)}
width = 416
height = 416


async def detect_thread(my_net, frame, my_width, my_height, thresh, loop):
    return await loop.run_in_executor(None, partial(detect_block, my_net, frame, my_width, my_height,
                                                    thresh))


def detect_block(my_net, frame, my_width, my_height, thresh):
    ...
    detections = dn.detect_image(my_net, darknet_image, thresh=thresh)
    ...
    async def task2(self):
        ...
        while self.running_level1:
            ...
            for nkey, network in net.items():
                tasks.append(detect_thread(network, frame_rgb, width,
                                           height, self.th, self.loop))
